...
// Структура для декодування JSON-вхідних даних
 type Input struct {
	Values []float64 `json:"values"`
}

// Збереження даних про види палива та їхні характеристики
...
func calculateEmission(fuelId string, value float64) ([]float64, error) {
	...
	// Розрахунок коефіцієнта емісії твердих частинок (г/ГДж)
...
func calculateTask1(coal, fuelOil, naturalGas float64) string {
	...
	if ktvEtv, err := calculateEmission("coal", coal); err == nil {
		output += fmt.Sprintf("Показник емісії твердих частинок при спалюванні вугілля: %.2f г/ГДж\n", ktvEtv[0])
		output += fmt.Sprintf("Валовий викид при спалюванні вугілля: %.2f т\n", ktvEtv[1])
	}

	if ktvEtv, err := calculateEmission("fuel_oil", fuelOil); err == nil {
		output += fmt.Sprintf("Показник емісії твердих частинок при спалюванні мазуту: %.2f г/ГДж\n", ktvEtv[0])
		output += fmt.Sprintf("Валовий викид при спалюванні мазуту: %.2f т\n", ktvEtv[1])
	}

	if ktvEtv, err := calculateEmission("natural_gas", naturalGas); err == nil {
		output += fmt.Sprintf("Показник емісії твердих частинок при спалюванні природного газу: %.2f г/ГДж\n", ktvEtv[0])
		output += fmt.Sprintf("Валовий викид при спалюванні природного газу: %.2f т\n", ktvEtv[1])
	}
...
func calculator1Handler(w http.ResponseWriter, r *http.Request) {
	...
	// Виконання розрахунків і повернення результату
...
// Головна функція, яка запускає HTTP-сервер
func main() {
	http.Handle("/", http.FileServer(http.Dir("./static")))
	http.HandleFunc("/api/calculator1", calculator1Handler)

	fmt.Println("Сервер працює за адресою http://localhost:8082")
